...
def history(update: Update, context: CallbackContext) -> None:
    ...
    for message in messages:
        ...
        # Добавляем к сообщению вложения (File ID) без отправки фото
        related_attachments = [a for a in attachments if a[1] == message_id]
        for attachment in related_attachments:
            file_id = attachment[2]
            response += f'📷 Вложение №{attachment_index} (File ID: {file_id})\n'
            attachment_index += 1
