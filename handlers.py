            ...
            if chat_member.status in ("member", "administrator", "creator"):
                ...
                if ticket:
                    ...
                    notification_text = (f'🔔 Создано обращение №{ticket_id} от пользователя @{update.message.from_user.username} '
                                        f'(Telegram ID: {update.message.from_user.id}): {message_text}')
                    update.message.reply_text('✉️ Ваше сообщение было отправлено агентам поддержки, ожидайте ответа')
            else:
                keyboard = [
                    [InlineKeyboardButton("👉 Подписаться на канал", url=f"https://t.me/gta_mn")],
                    [InlineKeyboardButton("🚀 Запустить бота", callback_data='start')],]
...
def handle_photo(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if ticket:
            ...
            notification_text = (f'📷 Добавлена фотография к обращению №{ticket_id} от пользователя @{update.message.from_user.username} (Telegram ID: {update.message.from_user.id})')
        else:
            ticket_id = create_ticket(user_id, '1', '*Вложение*', update.message.from_user.username)
            add_attachment(ticket_id, photo_file)
            notification_text = (f'📷 Создано обращение с фотографией №{ticket_id} от пользователя @{update.message.from_user.username} (Telegram ID: {update.message.from_user.id})')
            update.message.reply_text('✉️ Агенты поддержки получили Ваше обращение, пожалуйста, ожидайте ответа')
...
def history(update: Update, context: CallbackContext) -> None:
    ...
    if messages:
        ...
        for message in messages:
            ...
            response += f'[{timestamp_gmt3}] — {sender}: {message[3]}\n'
