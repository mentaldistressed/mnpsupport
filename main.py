...
from handlers import fileid, start, handle_message, answer_ticket, change_ticket_status, view_tickets, button_callback, history, handle_photo, ansid, ping, handle_video, reboot, block, stats, edit
import os
import threading
import time
import sys
...
def notify_agents(context: CallbackContext):
    context.bot.send_message(chat_id=agents_chat_id, text="✅ Выполнен запуск/перезагрузка поллинга, текущая версия: mwTOSW40-release-120124. Резервные копирования: включены")

def stop_polling_notification(updater: Updater) -> None:
    updater.bot.send_message(
        chat_id=agents_chat_id,
        text="🔴 Выполнена остановка поллинга, текущая версия: mwTOSW40-release-120124."
    )
